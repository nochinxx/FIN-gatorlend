...
function formatTokenizationStatus(status: string) {
  if (status === "mock_tokenized") return "Not minted on-chain";
  if (status === "xrpl_testnet_minted" || status === "verified_on_chain") return "Minted on XRPL";
  ...
  return (
    <main style={{ maxWidth: 960, margin: "0 auto", padding: "3rem 1.5rem 4rem" }}>
      <style>{`
        ...
        .listing-detail-gallery { display: grid; gap: 0.85rem; }
        .listing-detail-side { display: grid; gap: 1rem; }
        .listing-request-actions { display: grid; gap: 0.75rem; width: 100%; }
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: "0.75rem", alignItems: "flex-start" }}>
          ...
          <Link href="/listings/new" style={{ color: "#4ade80" }}>Create listing</Link>
        </div>
      </div>
      ...
      <section className="listing-detail-grid" style={{ marginTop: "2rem" }}>
        <div className="listing-detail-gallery">
          <div style={{ overflow: "hidden", borderRadius: 20, border: "1px solid #242424", position: "relative", aspectRatio: "4 / 5", background: "#1a1a1a" }}>
            {coverImage ? (
              <Image
                src={coverImage}
                alt={listing.title}
          ...
          {galleryImages.length > 0 ? (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: "0.65rem" }}>
              {galleryImages.map((imageUrl, index) => (
                <div
                  key={`${imageUrl}-${index}`}
                  style={{ overflow: "hidden", borderRadius: 16, border: "1px solid #242424", position: "relative", aspectRatio: "1 / 1", background: "#1a1a1a" }}
                >
                  <Image
                    src={imageUrl}
        ...
        <div className="listing-detail-side">
          {isOwner ? (
            <article style={{ padding: "1.5rem", borderRadius: 20, border: "1px solid #242424", background: "#111111" }}>
              ...
              <p style={{ margin: "0.35rem 0 1rem", color: "#9a9a9a", lineHeight: 1.6 }}>
              ...
              <form action={uploadListingImagesAction} style={{ display: "grid", gap: "0.85rem" }}>
                ...
                <label style={{ display: "grid", gap: "0.35rem" }}>
                  ...
                  <input type="file" name="images" multiple accept="image/jpeg,image/png,image/webp" style={inputStyle} />
                </label>
          ...
          <article style={{ padding: "1.5rem", borderRadius: 20, border: "1px solid #242424", background: "#111111" }}>
            <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap", alignItems: "center" }}>
              <span style={{ padding: "0.4rem 0.7rem", borderRadius: 999, background: "rgba(255,255,255,0.08)", color: "#aaaaaa" }}>
              ...
              <span style={{
                ...
                borderRadius: 999,
                background: listing.xrpl_token_id ? "rgba(59,130,246,0.12)" : "rgba(255,255,255,0.08)",
                color: listing.xrpl_token_id ? "#93c5fd" : "#aaaaaa"
              }}>
            ...
            {listing.description ? (
              <p style={{ margin: "1rem 0 0", color: "#9a9a9a", lineHeight: 1.6 }}>{listing.description}</p>
            ) : null}
          </article>

          <article style={{ padding: "1.5rem", borderRadius: 20, border: "1px solid #242424", background: "#111111" }}>
            <h2 style={{ marginTop: 0, fontSize: "1.1rem" }}>How this transfer works</h2>
            <div style={{ display: "grid", gap: "0.45rem", color: "#9a9a9a" }}>
          ...
          {!isOwner ? (
            <article id="request-form" style={{ padding: "1.5rem", borderRadius: 20, border: "1px solid #242424", background: "#111111", scrollMarginTop: "6rem" }}>
              <h2 style={{ marginTop: 0 }}>Request this listing</h2>
              {relatedRequest ? (
                <div style={{ display: "grid", gap: "0.6rem", color: "#9a9a9a" }}>
                  ...
                  {relatedRequest.status === "pending" ? (
                    <form action={cancelRequestAction}>
                      ...
                      <FormSubmitButton
                        ...
                        style={{ width: "fit-content", padding: "0.85rem 1rem", borderRadius: 999, border: "1px solid #2a2a2a", background: "#141414", color: "#e5e5e5", fontWeight: 700 }}
                      >
                        Cancel request
                      </FormSubmitButton>
                    </form>
                ...
                <form action={requestListingAction} style={{ display: "grid", gap: "0.85rem" }}>
                  ...
                  <label style={{ display: "grid", gap: "0.35rem" }}>
                    ...
                    <input name="handoff_location" style={inputStyle} />
                  </label>
                  <label style={{ display: "grid", gap: "0.35rem" }}>
                    <span>Availability note</span>
                    <input name="availability_note" placeholder="Weekday afternoon, library area" style={inputStyle} />
                  </label>
                  <label style={{ display: "grid", gap: "0.35rem" }}>
                    <span>Exchange note</span>
                    <input name="payment_method" style={inputStyle} />
                  </label>
                  <FormSubmitButton
                    pendingLabel="Sending request..."
                    style={{ width: "fit-content", padding: "0.85rem 1rem", borderRadius: 999, border: 0, background: "#16a34a", color: "#ffffff", fontWeight: 700 }}
                  >
                    Send request
                  </FormSubmitButton>
                </form>
        ...
        <section style={{ marginTop: "1.5rem", display: "grid", gap: "1rem" }}>
          ...
          {requests.length === 0 ? (
            ...
            requests.map((request) => {
              ...
              return (
                <article key={request.id} style={{ padding: "1.5rem", borderRadius: 20, border: "1px solid #242424", background: "#111111" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem", flexWrap: "wrap" }}>
                    <div>
                      ...
                      <p style={{ margin: "0.35rem 0 0", fontSize: 14, color: "#9a9a9a" }}>
                    ...
                    <div className="listing-request-actions">
                      {request.status === "pending" ? (
                        <div style={{ width: "100%", maxWidth: 420, display: "grid", gap: "0.65rem" }}>
                          <textarea
                            ...
                            style={{ width: "100%", padding: "0.8rem 0.9rem", borderRadius: 14, border: "1px solid #2a2a2a", background: "#141414", color: "#e5e5e5", resize: "vertical" }}
                          />
                      ...
                      {request.status === "accepted" ? (
                        <div style={{ display: "grid", gap: "0.5rem" }}>
                          <p style={{ margin: 0, maxWidth: 280, color: "#9a9a9a" }}>
                          ...
                          <form action={confirmHandoffAction}>
                            ...
                            <FormSubmitButton pendingLabel="Confirming..." style={{ padding: "0.75rem 0.95rem", borderRadius: 999, border: 0, background: "#111111", color: "#ffffff", fontWeight: 700 }}>
                              Mark as handed off
                            </FormSubmitButton>
                          </form>
                        </div>
